...
func main() {
	...
	sessionPtr := flag.String("session", "cl_default_session", "指定会话 ID，支持断点续传")
	flag.Parse()
	...
	//	获取持久化Session
	sess := ctxpkg.GlobalSessionManager.GetOrCreate(*sessionPtr, workDir)
	// 【全息监控装配】：用 Cost Tracker 将真实大脑包裹起来
	trackerProvider := observability.NewCostTracker(realProvider, modelName, sess)
	...
	eng := engine.NewAgentEngine(trackerProvider, registry, false, true)
