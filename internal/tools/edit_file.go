...
func fuzzyReplace(originalContent, oldText, newText string) (string, error) {
	...
	trimmedOld := strings.TrimSpace(normalizedContent)
	if trimmedOld != "" {
		...
		if count == 1 {
			...
			return strings.Replace(trimmedOld, normalizedOld, newText, 1), nil
		}
	}
...
func lineByLineReplace(content string, oldText string, newText string) (string, error) {
	...
	matchStartIndex := -1
	matchEndIndex := -1
	for i := 0; i < len(contentLines)-len(oldLines)+1; i++ {
		isMatch := true
...
func (t *EditFileTool) Execute(ctx context.Context, args json.RawMessage) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("多级模糊替换失败: %w", err)
	}
	//	3.将新内容安全写回磁盘
